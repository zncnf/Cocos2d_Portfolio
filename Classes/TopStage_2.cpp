...
Scene * TopStage_2::createScene()
{
	return TopStage_2::create();
}

bool TopStage_2::init()
{
	if (!Scene::init()) {
		return false;
	}
	...
	_map = Sprite::create("Map/저주받은 신전.png");
	...
	K_listner->onKeyPressed = CC_CALLBACK_2(TopStage_2::onKeyPressed, this);
	K_listner->onKeyReleased = CC_CALLBACK_2(TopStage_2::onKeyReleased, this);
	...
	startCount->runAction(Sequence::create(
		Spawn::create(
			...
			Repeat::create(Sequence::create(
				...
				nullptr), 4),
			nullptr),
		CallFunc::create(CC_CALLBACK_0(TopStage_2::setGame, this)),
...
void TopStage_2::tick(float delta)
{
	if (!player->getIsDead()) {
		_time += delta;
		if (player->getBestTime(1) < _time) {
			...
			_bestTimeLabel->setString(StringUtils::format("BEST TIME   %02d : %02d", (int)player->getBestTime(1) / 60, (int)player->getBestTime(0) % 60));
		}
	}
	else if (!_isGameSet) {
		_isGameSet = true;
		this->scheduleOnce(schedule_selector(TopStage_2::gameOver), 2);
	}
...
void TopStage_2::onKeyPressed(EventKeyboard::KeyCode keyCode, Event * event)
{
...
void TopStage_2::onKeyReleased(EventKeyboard::KeyCode keyCode, Event * event)
{
	player->onKeyReleased(keyCode, event);
}

void TopStage_2::onEnter()
{
	Scene::onEnter();

	auto listener = EventListenerTouchOneByOne::create();
	listener->setSwallowTouches(true);
	listener->onTouchBegan = CC_CALLBACK_2(TopStage_2::onTouchBegan, this);
	listener->onTouchMoved = CC_CALLBACK_2(TopStage_2::onTouchMoved, this);
	listener->onTouchEnded = CC_CALLBACK_2(TopStage_2::onTouchEnded, this);
	listener->onTouchCancelled = CC_CALLBACK_2(TopStage_2::onTouchCancelled, this);

	_eventDispatcher->addEventListenerWithSceneGraphPriority(listener, this);
}

void TopStage_2::onExit()
{
...
bool TopStage_2::onTouchBegan(Touch * touch, Event * event)
{
...
void TopStage_2::onTouchMoved(Touch * touch, Event * event)
{
...
void TopStage_2::onTouchEnded(Touch * touch, Event * event)
{
	cuey->glview()->setCursor("Cursor_up.png");
}

void TopStage_2::onTouchCancelled(Touch * touch, Event * event)
{

}

void TopStage_2::setGame()
{
	player->setLayer(_layer, true);
	this->schedule(schedule_selector(TopStage_2::tick));
}
