...
Equip::Equip()
{
	...
	cache->addSpriteFramesWithFile("Equip/Weapon/≈¬±ÿ∫Œ√§.plist");
	cache->addSpriteFramesWithFile("Equip/Armor/ƒø¥◊∞¯∞Ì ±≥∫π.plist");
	cache->addSpriteFramesWithFile("Equip/Armor/commander.plist");
	cache->addSpriteFramesWithFile("Equip/Armor/≥™¿Ã∆Æ ∏≈¡ˆº«.plist");
	cache->addSpriteFramesWithFile("Equip/Shoes/luminous.plist");
	cache->addSpriteFramesWithFile("Equip/Shoes/π´«—¿« ∆˜≈Ê Ω≈πﬂ.plist");
	cache->addSpriteFramesWithFile("Equip/Shoes/Ω¥∆€ ∫ŒΩ∫≈Õ øÓµø»≠.plist");
...
void Equip::setWeapon(String name, int n)
{
	...
	else if (name.compare("≈¬±ÿ∫Œ√§") == 0) {
		_myWeapon[n]->code = "≈¬±ÿ∫Œ√§";
...
void Equip::setArmor(String name, int n)
{
	...
	if (name.compare("ƒø¥◊∞¯∞Ì ±≥∫π") == 0) {
		_myArmor[n]->code = "ƒø¥◊∞¯∞Ì ±≥∫π";
		_myArmor[n]->life = 1.0f;
	}
	else if (name.compare("ªÁ∑…∞¸ Ω¥∆Æ") == 0) {
		_myArmor[n]->code = "commander";
		_myArmor[n]->life = 2.0f;
	}
	else if (name.compare("≥™¿Ã∆Æ ∏≈¡ˆº«") == 0) {
		_myArmor[n]->code = "≥™¿Ã∆Æ ∏≈¡ˆº«";
...
void Equip::setShoes(String name, int n)
{
	...
	else if (name.compare("π´«—¿« ∆˜≈Ê Ω≈πﬂ") == 0) {
		_myShoes[n]->code = "π´«—¿« ∆˜≈Ê Ω≈πﬂ";
		_myShoes[n]->speed = 1.5f;
	}
	else if (name.compare("Ω¥∆€ ∫ŒΩ∫≈Õ øÓµø»≠") == 0) {
		_myShoes[n]->code = "Ω¥∆€ ∫ŒΩ∫≈Õ øÓµø»≠";
