	...
	startCount->runAction(Sequence::create(
		Spawn::create(
			...
			Repeat::create(Sequence::create(
				...
				nullptr), 4),
			nullptr),
		CallFunc::create(CC_CALLBACK_0(TopStage_3::setGame, this)),
...
void TopStage_3::tick(float delta)
{
	if (!player->getIsDead()) {
		...
		if (player->getBestTime(2) < _time) {
			...
			_bestTimeLabel->setString(StringUtils::format("BEST TIME   %02d : %02d", (int)player->getBestTime(2) / 60, (int)player->getBestTime(0) % 60));
		}
	}
	else if(!_isGameSet) {
		_isGameSet = true;
		this->scheduleOnce(schedule_selector(TopStage_3::gameOver), 2);
	}
